...
# If the Heroku Postgres DATABASE_URL has changed, this will update accordingly the BACKEND_STORE_URI in the app config vars :
...
docker run -it\
 ...
 $IMAGE_NAME python $PYTHON_SCRIPT_PATH "$@"